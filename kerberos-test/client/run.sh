...
java -Djava.security.auth.login.config=$JAAS_FILE -jar /tmp/akka-kerberos-assembly-0.1.jar &
...
if [ "$ANSWER" = "Hello $KADMIN_PRINCIPAL" ]; then
	echo "test passed"
else
	echo "test failed:"
	echo "got: $ANSWER"
	echo "expected: Hello $KADMIN_PRINCIPAL"
	exit 1
fi
